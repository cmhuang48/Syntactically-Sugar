  ...
  if (token) {
    ...
    return async (dispatch) => {
      ...
      if (!localStorage.length) {
        ...
        const removeExistingLineItem = lineItems.filter(
          (ele) => ele.id !== 2 && ele.id !== 3 && ele.id !== 1
        );
        const productPrice = {
          1: 45,
          2: 60,
        ...
        const lineItemId = removeExistingLineItem.map(
          (lineItem) => lineItem.productId
        );
        const lineItemQuantity = removeExistingLineItem.map(
          (lineItem) => lineItem.quantity
        );
        const existingItemPrice = lineItemId.map((ele) => productPrice[ele]);
        orderTotal = lineItemQuantity.reduce((acc, next, i) => {
          return acc + next * existingItemPrice[i];
        }, 0);
      } else {
        lineItems = (
          await axios.put(
